...
struct bucket {
  ...
  DEVICE_QUALIFIER
  void load(std::memory_order order = std::memory_order_seq_cst) {
  ...
  bool weak_cas_at_location(const pair_type& pair,
    ...
    bool cas_success   = ptr_[location].compare_exchange_weak(expected, desired, success, failure);
    return cas_success;
  }
  ...
  bool strong_cas_at_location(const pair_type& pair,
    ...
    pair_type expected = sentinel;
    pair_type desired  = pair;
    bool cas_success = ptr_[location].compare_exchange_strong(expected, desired, success, failure);
    return cas_success;
  }
